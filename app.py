    ...
    if results and len(results) > 0:
        for box, cls_id, conf in zip(results[0].boxes.xyxy, results[0].boxes.cls, results[0].boxes.conf):
            ...
            if class_name == target_card and conf > CONF_THRESHOLD:
                ...
                font_scale = 0.7
                thickness = 2
                text_size, _ = cv2.getTextSize(display_name, font, font_scale, thickness)
                ...
                # outline
                cv2.putText(overlay_text, display_name, (text_x, text_y), font, font_scale, (0, 0, 0), thickness + 2, cv2.LINE_AA)
                # main text
                cv2.putText(overlay_text, display_name, (text_x, text_y), font, font_scale, (0, 0, 255), thickness, cv2.LINE_AA)
...
def process_frame():
    target_card = request.form.get('target_card', 'Jd')
